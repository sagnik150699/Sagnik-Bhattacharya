# Add FAQPage schema to top blog posts for GEO (Generative Engine Optimization)
# This script injects FAQPage JSON-LD into existing blog HTML files

$blogDir = "c:\Workspace\Sagnik Bhattacharya\public\blog"
...
$faqs = @{
    ...
    "excel-ai-prompts" = @(
        @{q="What are the best AI prompts for Excel?"; a="The most effective AI prompts for Excel include your data context: column headers, sample data, and desired output. For example: 'My data has columns Date, Region, Product, Revenue in A1:D500. Write a formula to find the top 3 products by total revenue per region.' Context-rich prompts produce accurate formulas 90% of the time, while vague prompts like 'write a sales formula' often produce unusable results."},
        @{q="How should I prompt ChatGPT or Claude for Excel formulas?"; a="Follow this structure: (1) Describe your data layout with column letters and headers, (2) Specify the cell where the formula should go, (3) Explain the expected output clearly, (4) Mention any constraints like avoiding VBA or needing backward compatibility. Example: 'In cell E2, write a formula that looks up the value in B2 against the range Sheet2!A:C and returns the matching value from column C. Use XLOOKUP.'"},
        @{q="Can AI write VLOOKUP and XLOOKUP formulas for me?"; a="Yes, all major AI tools (ChatGPT, Claude, Gemini, Copilot) can write VLOOKUP and XLOOKUP formulas accurately. They can also suggest which lookup function is best for your use case, convert VLOOKUP formulas to XLOOKUP, and handle complex multi-criteria lookups. Always test the formula with your actual data before applying it widely."}
    )
    "chatgpt-vs-claude-vs-copilot-vs-gemini-excel" = @(
        @{q="Which AI is best for Excel in 2026?"; a="It depends on the task. Microsoft Copilot is best for in-app Excel automation (it works directly inside Excel). ChatGPT is best for end-to-end analysis with file uploads via Code Interpreter. Claude is best for complex formula logic, debugging, and clear explanations. Google Gemini is the best free option for formula generation. For most users, ChatGPT or Claude for formula writing plus Copilot for in-app tasks is the strongest combination."},
    ...
    "mastering-pivot-tables" = @(
        @{q="How do I create a pivot table in Excel?"; a="Select your data range (ensure it has headers), go to Insert > PivotTable, choose where to place it, then drag fields into the Rows, Columns, Values, and Filters areas. The key to good pivot tables: use Excel Tables (Ctrl+T) as your source so new data is automatically included, keep your source data clean with no blank rows or merged cells, and use one row per record."},
        @{q="When should I use a pivot table instead of formulas in Excel?"; a="Use pivot tables when you need to quickly summarise, group, or cross-tabulate large datasets without writing formulas. Pivot tables are best for exploratory analysis — when you want to slice data by different dimensions interactively. Use formulas (SUMIFS, GROUPBY, PIVOTBY) when you need the output to be formula-driven, auto-updating, and part of a structured report or dashboard."}
    )
    "dynamic-dashboards" = @(
        @{q="How do I build an interactive dashboard in Excel without VBA?"; a="Use a combination of Excel Tables, pivot tables or dynamic array formulas (FILTER, SORT, GROUPBY), data validation dropdowns for interactivity, and charts linked to the filtered data. Slicers connected to pivot tables provide point-and-click filtering. Use named ranges or structured table references so everything updates automatically when new data is added."},
        @{q="Can I create a professional dashboard in Excel without coding?"; a="Yes. Excel's built-in features — pivot tables, slicers, charts, conditional formatting, data validation dropdowns, and sparklines — are enough to build professional, interactive dashboards without any VBA or coding. The key is clean data structure, consistent formatting, and linking charts to filtered or summarised data ranges."}
    )
    "excel-vs-google-sheets" = @(
        @{q="Is Excel or Google Sheets better in 2026?"; a="Excel is better for power users who need advanced formulas (LAMBDA, GROUPBY, PIVOTBY), Power Query, Power Pivot, VBA macros, large datasets (1M+ rows), and desktop performance. Google Sheets is better for real-time collaboration, simplicity, free access, and web-native workflows. For professional data analysis and financial modelling, Excel is the stronger choice. For team collaboration on simpler spreadsheets, Google Sheets wins."},
        @{q="Should I use Excel or Google Sheets for work?"; a="If your work involves complex data analysis, financial models, large datasets, or automation with macros, use Excel. If your work is primarily collaborative (shared trackers, simple reports, team templates) and your organisation uses Google Workspace, use Google Sheets. Many professionals use both — Google Sheets for shared lightweight work and Excel for heavy analysis."}
    )
    "agent-mode-in-excel" = @(
    ...
    "flutter-renderflex-overflow-row-listview" = @(
        @{q="How do I fix RenderFlex overflowed in Flutter?"; a="The most common fix is to wrap the overflowing child inside a Flexible or Expanded widget, which tells Flutter to constrain the child to the available space. If the overflow happens in a Row inside a ListView, wrap the Row's text or wide children with Flexible(child: Text(..., overflow: TextOverflow.ellipsis)). Other fixes include using Wrap instead of Row, adding SingleChildScrollView, or constraining with SizedBox."},
        @{q="Why does my Flutter Row overflow inside a ListView?"; a="A Row inside a ListView has unbounded width from the Row's children but constrained width from the ListView. If the Row's children (especially Text widgets) try to take more horizontal space than available, Flutter cannot lay them out and throws a RenderFlex overflow error. The fix is to wrap wide children with Flexible or Expanded to make them respect the available width."}
    )
}
...
foreach ($slug in $faqs.Keys) {
    $filePath = Join-Path $blogDir "$slug.html"
    
    if (-not (Test-Path $filePath)) {
        Write-Host "SKIP: $slug.html not found" -ForegroundColor Yellow
        $errorCount++
        continue
    }
    ...
    if ($content -match 'FAQPage') {
        Write-Host "SKIP: $slug.html already has FAQPage schema" -ForegroundColor Yellow
        continue
    }
    
    # Build FAQ JSON-LD
    $mainEntity = @()
    foreach ($faq in $faqs[$slug]) {
        $mainEntity += @{
            '@type' = 'Question'
            'name' = $faq.q
            'acceptedAnswer' = @{
    ...
    $faqJson = $faqSchema | ConvertTo-Json -Depth 10 -Compress:$false
    
    $scriptBlock = "  <script type=`"application/ld+json`">`n$faqJson`n  </script>"
    
    # Find the insertion point: right before the first <style> tag in the head
    # We insert after the last </script> before <style>
    ...
    if ($styleIndex -eq -1) {
        Write-Host "SKIP: $slug.html - could not find <style> tag" -ForegroundColor Red
        $errorCount++
        continue
    }
    
    # Find the last </script> before <style>
    $searchArea = $content.Substring(0, $styleIndex)
    $lastScriptEnd = $searchArea.LastIndexOf('</script>')
    
    if ($lastScriptEnd -eq -1) {
        Write-Host "SKIP: $slug.html - could not find </script> before <style>" -ForegroundColor Red
        $errorCount++
        continue
    }
    
    # Insert after </script> (which is 9 characters long)
    $insertPos = $lastScriptEnd + 9  # length of '</script>'
    
    $newContent = $content.Substring(0, $insertPos) + "`n" + $scriptBlock + $content.Substring($insertPos)
    
    Set-Content $filePath $newContent -NoNewline -Encoding UTF8
    
    $processedCount++
    Write-Host "OK: $slug.html - added $($faqs[$slug].Count) FAQs" -ForegroundColor Green
}

Write-Host "`nDone. Processed: $processedCount, Errors: $errorCount" -ForegroundColor Cyan
