...
NEW_POSTS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "instead": {"paragraphs": ["To set up Gemma 4 locally, see <a href=\"/blog/run-gemma-4-own-machine\">running Gemma 4 on your own machine</a>. For using Gemma 4 in your code editor, see <a href=\"/blog/gemma-4-vscode\">Gemma 4 in VS Code</a>."]},
        "related_new": ["run-gemma-4-own-machine", "open-models-private-ai-apps", "local-ai-own-files", "choose-open-models-vs-api-models"],
    ...
    },
    {
        ...
        "related_new": ["gemma-4-local-ai-workflows", "open-models-private-ai-apps", "local-ai-own-files", "choose-open-models-vs-api-models"],
    ...
    {
        "slug": "open-models-private-ai-apps",
        "title": "How to Use Open Models for Private AI Apps",
        "category": "AI",
        "description": "Learn how to build AI applications with open-weight models like Gemma, Llama, and Mistral that keep all data on your own infrastructure.",
        "keywords": ["open models private AI", "private AI apps", "self-hosted AI", "open-weight models", "data privacy AI"],
        "primary_intent": "Build private AI applications using open-weight models.",
        "search_type": "evergreen",
        "intro": [
            "Some data should never leave your infrastructure — medical records, legal documents, proprietary code, financial data. Open-weight models let you build AI applications that process this data without any external API calls.",
            "This guide covers the practical side of building private AI apps with open models: which models to choose, how to host them, and where the trade-offs are."
        ],
        "quick_answer": "Host an open-weight model (Gemma, Llama, Mistral) on your own infrastructure using Ollama, vLLM, or a cloud GPU. Build your app against the local API. Data never leaves your control, and there are no per-token costs.",
        "use_when": [
            "Your data cannot be sent to external AI providers for legal or compliance reasons.",
            "You want to eliminate per-token API costs for high-volume applications.",
            "You need full control over the model, including fine-tuning and customisation.",
        ],
        "sections": [
            {"heading": "Why open models for privacy", "paragraphs": ["When you use a cloud AI API, your data goes to someone else's servers. Most providers say they do not train on your data, but the data still leaves your network. For some organisations and data types, that is not acceptable.", "Open-weight models give you full control. You download the model weights, run them on your hardware, and the data never touches an external server."]},
            {"heading": "Choosing the right model", "paragraphs": ["The main open model families are Gemma (Google), Llama (Meta), and Mistral. Each has strengths: Gemma is efficient at smaller sizes, Llama offers the largest open models, and Mistral balances quality and speed.", "For most private AI apps, start with the smallest model that handles your task well. A 7B-12B model is enough for summarisation, extraction, and classification. Use larger models only if quality genuinely improves."], "table": {"headers": ["Model", "Best For", "Sizes", "Licence"], "rows": [["Gemma 4", "Efficient local use, code", "1B-27B", "Open weights"], ["Llama 4", "General tasks, large context", "8B-405B", "Open weights"], ["Mistral", "Fast inference, European compliance", "7B-123B", "Open weights"]]}},
            {"heading": "Hosting options", "paragraphs": ["For development and small-scale use, run the model locally with Ollama or LM Studio. For production, use vLLM or TGI on a cloud GPU (AWS, GCP, Azure) within your own VPC.", "The key is that 'cloud GPU' still means 'your infrastructure' if you are using your own cloud account with proper network isolation. The model runs in your VPC, not on the AI provider's servers."]},
            {"heading": "Building the application", "paragraphs": ["Your app code is the same regardless of whether you use a local model or a cloud API — you call an HTTP endpoint and get back a response. Most open model servers support the OpenAI API format, so you can use any OpenAI-compatible client library.", "The main difference is performance characteristics. Local models are slower and have lower throughput than cloud APIs. Design your app to handle 2-10 second response times instead of sub-second."]},
            {"heading": "Trade-offs and limitations", "paragraphs": ["Open models are not as capable as the best cloud models (GPT-4o, Claude Opus). For tasks where quality is critical, test whether the open model is good enough before committing.", "You also take on operational responsibility — model updates, GPU provisioning, monitoring, and scaling. This is real work that cloud APIs handle for you."]},
        ],
        "example": {"heading": "Worked example: internal document analysis tool", "paragraphs": ["A law firm builds an internal tool that analyses contracts for specific clauses. They host Gemma 4 12B on an internal server. Lawyers upload contracts through a web interface, the model analyses them locally, and results never leave the firm's network. Per-document cost: electricity only."]},
        "mistakes": ["Assuming open models match cloud model quality for all tasks — test first.", "Not accounting for operational costs (GPU hardware, maintenance, monitoring).", "Using unquantised models when quantised versions would perform nearly as well with less hardware."],
        "instead": {"paragraphs": ["For setting up Gemma 4 specifically, see <a href=\"/blog/run-gemma-4-own-machine\">running Gemma 4 on your own machine</a>. For deciding between open and API models, see <a href=\"/blog/choose-open-models-vs-api-models\">choosing between open and API models</a>."]},
        "related_new": ["run-gemma-4-own-machine", "local-ai-own-files", "choose-open-models-vs-api-models", "gemma-4-local-ai-workflows"],
        "related_old": ["gemma-4-vs-paid-ai-models"],
        "related_intro": "These guides cover local model setup, file processing, and model selection.",
        "cover_hook": "Build AI apps where sensitive data never leaves your infrastructure using open-weight models.",
        "cover_cta": "Build Private AI Apps",
        "cover_keywords": ["Open Models", "Private", "AI"],
        "cover_cue": "infrastructure diagrams, privacy shields, and self-hosted deployment visuals",
        "image_alt": "Coding Liquids blog cover featuring Sagnik Bhattacharya for using open models for private AI apps.",
        "sources": [],
    },
    {
        ...
        "instead": {"paragraphs": ["If you want to build a full RAG system over your documents, see <a href=\"/blog/build-rag-app-own-documents\">building a RAG app</a>. For setting up the local model itself, see <a href=\"/blog/run-gemma-4-own-machine\">running Gemma 4 on your own machine</a>."]},
        "related_new": ["run-gemma-4-own-machine", "build-rag-app-own-documents", "gemma-4-local-ai-workflows", "open-models-private-ai-apps"],
    ...
    },
    {
        ...
        "related_new": ["improve-rag-reranking", "chunk-documents-rag", "local-ai-own-files", "open-models-private-ai-apps"],
        "related_old": [],
    ...
    },
    {
        ...
        "related_new": ["chunk-documents-rag", "build-rag-app-own-documents", "evaluate-ai-outputs-real-apps", "open-models-private-ai-apps"],
        "related_old": [],
    ...
    },
    {
        ...
        "instead": {"paragraphs": ["For choosing between open models and API models overall, see <a href=\"/blog/choose-open-models-vs-api-models\">choosing between open and API models</a>. For running models locally to eliminate API costs entirely, see <a href=\"/blog/run-gemma-4-own-machine\">running Gemma 4 locally</a>."]},
        "related_new": ["choose-open-models-vs-api-models", "run-gemma-4-own-machine", "background-jobs-ai-apps", "reasoning-summaries-production-ai"],
        "related_old": ["gemma-4-vs-paid-ai-models"],
    ...
    },
    {
        ...
        "instead": {"paragraphs": ["For running open models locally, see <a href=\"/blog/run-gemma-4-own-machine\">running Gemma 4 on your own machine</a>. For reducing API costs without switching models, see <a href=\"/blog/cut-ai-api-costs-caching-routing\">cutting AI API costs</a>."]},
        "related_new": ["run-gemma-4-own-machine", "cut-ai-api-costs-caching-routing", "open-models-private-ai-apps", "gemma-4-local-ai-workflows"],
    ...
    },
    {
        ...
        "instead": {"paragraphs": ["For consistent characters across clips, see <a href=\"/blog/consistent-characters-seedance\">consistent characters in Seedance 2.0</a>. For using reference images, see <a href=\"/blog/seedance-reference-images-characters\">reference images for characters</a>."]},
        "related_new": ["consistent-characters-seedance", "seedance-reference-images-characters", "better-prompts-seedance", "seedance-dreamina-guide"],
        "related_old": [],
    ...
    {
        "slug": "seedance-reference-images-characters",
    ...
    },
    {
        ...
        "instead": {"paragraphs": ["For using reference images specifically, see <a href=\"/blog/seedance-reference-images-characters\">reference images for characters</a>. For better prompts in general, see <a href=\"/blog/better-prompts-seedance\">better prompts for Seedance 2.0</a>."]},
        "related_new": ["seedance-reference-images-characters", "better-prompts-seedance", "seedance-dreamina-guide", "seedance-youtube-shorts"],
        "related_old": [],
